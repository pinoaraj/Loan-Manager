...
const Clients = () => {
    ...
    const { data: clientsData = { data: [], meta: {} } } = useQuery({
        ...
        queryFn: async () => {
            ...
            const response = await fetchWithAuth(`${API_URL}/clients?${query.toString()}`);
    ...
    const getClientDebt = (client) => {
        return (client.loans || [])
            ...
            .reduce((accumulator, loan) => accumulator + loan.amount, 0);
    };
                ...
                {clients.length === 0 ? (
                    <div className="col-span-full py-20 text-center bg-white dark:bg-slate-800 rounded-3xl border border-dashed border-slate-200">
                        ...
                        <p className="text-slate-500">
                            {searchTerm ? 'No se encontraron clientes para esa busqueda.' : 'No se encontraron clientes.'}
                        </p>
                    </div>
                ) : (
                    clients.map((client) => {
